...
export const CARD_NAMES: Record<CardId, string> = {
  ...
  pickpocket: "Pickpocket", explosion: "Explosion", shockwave: "Shockwave",
  chain_stun: "Chain Stun", sacred_spring: "Sacred Spring",
  cursed_blade: "Cursed Blade", blood_pact: "Blood Pact", soul_drain: "Soul Drain",
  cursed_heal: "Cursed Heal", betrayal: "Betrayal", plague: "Plague",
  sacrifice: "Sacrifice", hand_swap: "Hand Swap", cursed_mirror: "Cursed Mirror",
  curse_tax: "Curse Tax", blood_transfusion: "Blood Transfusion",
  ...
  overflow: "Overflow", echo_strike: "Echo Strike", celestial_mend: "Celestial Mend",
  soul_swap: "Soul Swap", armageddon: "Armageddon", void_rift: "Void Rift",
  judgement: "Judgement", time_warp: "Time Warp", phoenix_ember: "Phoenix Ember",
...
const CARD_RARITIES: Record<CardId, Rarity> = {
  ...
  soul_swap: "mythic", armageddon: "mythic", void_rift: "mythic",
  judgement: "mythic", time_warp: "mythic", phoenix_ember: "mythic",
  singularity: "void", dark_matter: "void", event_horizon: "void",
  neutron_star: "void", void_pulse: "void",
};

const RARITY_HEAL: Record<Rarity, number> = {
  common: 5, uncommon: 10, rare: 15, cursed: 20,
  corrupted: 22, legendary: 28, mythic: 32, void: 35,
};
...
function drawOne(exclude: CardId[] = []): CardId {
  ...
  if (finalPool.length === 0) {
    const all = (Object.values(POOLS) as CardId[][]).flat().filter(c => !exclude.includes(c));
...
interface Player {
  ...
  healBlocked: number;
  overflowPending: boolean;    // waiting to discard 2
  overflowDiscardCount: number; // how many still to discard
...
function getRoom(roomId: string): Room {
  if (!rooms.has(roomId)) {
    rooms.set(roomId, {
      ...
      gameOver: false, winnerId: null, isDraw: false,
      lastNarration: "", turnStartTime: 0,
    });
  }
...
// Replenish — does NOT add extra if weak_jab already drew
function replenishCard(player: Player, skipReplenish = false) {
  if (skipReplenish) return;
  player.hand.push(drawOne(player.hand as CardId[]));
}

function dealDamage(
  room: Room, actor: Player, target: Player,
  dmg: number, source: string, bypassShield = false
): number {
  if (target.eventHorizonActive && !bypassShield) {
    target.eventHorizonActive = false;
    actor.hp = Math.max(0, actor.hp - dmg);
    addLog(room, `🌑 EVENT HORIZON! ${source} collapses inward — ${dmg} damage hits ${actor.name}!`);
    return -2;
  }
  ...
  if (target.counterStanceActive && !bypassShield) {
    ...
    addLog(room, `🔄 ${target.name}'s COUNTER STANCE reflects ${dmg} + 12 bonus to them!`);
    return -1;
  }
  ...
  // Mana surge: double + 10 bonus
  if (actor.manaSurgeActive) {
    actor.manaSurgeActive = false;
    const surgeBonus = Math.min(actor.warCryBonus, Math.max(0, 30 - actor.warCryBonus));
    finalDmg = dmg * 2 + 10 + surgeBonus;
    actor.warCryBonus = 0;
    addLog(room, `✨ MANA SURGE: ${dmg} → ${finalDmg} damage!`);
  } else if (actor.warCryBonus > 0) {
    // War cry bonus capped at 30 total
    const bonus = Math.min(actor.warCryBonus, 30);
    finalDmg += bonus;
    addLog(room, `📯 WAR CRY adds ${bonus} bonus damage!`);
    actor.warCryBonus = 0;
  }
...
function applyStun(room: Room, actor: Player, target: Player, turns: number): boolean {
  if (actor.stunCooldown > 0) {
    addLog(room, `⚠ Stun on cooldown (${actor.stunCooldown} turn(s))! ${target.name} resists.`);
    return false;
  }
...
function checkDeath(room: Room, actorToken: string, targetToken: string): boolean {
  ...
  if (actorDead && targetDead) {
    ...
    addLog(room, `💀 BOTH WARRIORS FALL — IT'S A DRAW!`); return true;
  }
...
function startTurn(room: Room, token: string) {
  ...
  if (player.stunCooldown > 0) {
    player.stunCooldown--;
    if (player.stunCooldown === 0) addLog(room, `⚡ ${player.name}'s stun cooldown expired!`);
  }

  if (player.healBlocked > 0) {
    player.healBlocked--;
    if (player.healBlocked === 0) addLog(room, `🌡 ${player.name}'s heal block wore off!`);
  }

  if (player.neutronStarTicks > 0) {
    player.hp = Math.max(0, player.hp - 10);
    player.neutronStarTicks--;
    addLog(room, `⭐ NEUTRON STAR deals 10 radiation to ${player.name}! (${player.neutronStarTicks} ticks left)`);
    const oppToken = getOpponentToken(room, token);
    if (player.hp <= 0 && oppToken) { endGame(room, oppToken); return; }
  }

  if (player.regenStacks > 0) {
    const regen = player.regenStacks * 9;
  ...
  // Plague — asymmetric: self takes 5, opponent takes 8
  if (player.plagueStacks > 0) {
    player.hp = Math.max(0, player.hp - 5);
    player.plagueStacks = Math.max(0, player.plagueStacks - 1);
    addLog(room, `🦠 ${player.name} suffers plague! 5 damage (${player.plagueStacks} turns left)`);
    const oppToken = getOpponentToken(room, token);
    if (player.hp <= 0 && oppToken) { endGame(room, oppToken); return; }
  }

  if (player.selfPlagueStacks > 0) {
    player.selfPlagueStacks = Math.max(0, player.selfPlagueStacks - 1);
  }
...
function resolveCard(room: Room, actorToken: string, cardId: CardId) {
  ...
  switch (cardId) {
    ...
    case "weak_jab": {
      ...
      actor.lastCardDmg = Math.max(0, d);
      // Draw immediately — this replaces the replenish
    ...
    case "fumble": {
      ...
      actor.lastCardDmg = Math.max(0, d);
      if (Math.random() < 0.20 && actor.hand.length > 0) {
        const ridx = Math.floor(Math.random() * actor.hand.length);
        const lost = actor.hand.splice(ridx, 1)[0] as CardId;
        addLog(room, `🤦 ${actor.name} FUMBLED! ${actor.lastCardDmg} damage... and lost ${CARD_NAMES[lost]}!`);
        narration = `${actor.name} fumbles the strike! ${actor.lastCardDmg} damage but drops ${CARD_NAMES[lost]} from their hand!`;
      } else {
        addLog(room, `🤦 ${actor.name} fumbled awkwardly — ${actor.lastCardDmg} damage.`);
        narration = `${actor.name} fumbles but lands ${actor.lastCardDmg} damage.`;
      }
      break;
    }
    // ── UNCOMMON ──────────────────────────────────────────────
    case "heavy_blow": {
      ...
      if (d > 0) {
        ...
        narration = stunned ? `${actor.name} SMASHES ${target.name} for ${d} — STUNNED!` : `${actor.name} smashes for ${d} damage! Stun on cooldown.`;
      } else { addLog(room, `💥 Heavy Blow — blocked!`); narration = `${actor.name}'s heavy blow was deflected!`; }
      break;
    }
    case "heal": {
    ...
    case "shield": {
      if (actor.shield) {
        const h = applyHeal(room, actor, 10);
        addLog(room, `🛡 ${actor.name} already shielded — converted to ${h} HP heal!`);
        narration = `${actor.name} raises a second shield... already protected! Converts to ${h} HP.`;
      } else {
        actor.shield = true;
        addLog(room, `🛡 ${actor.name} raised a shield!`);
        narration = `${actor.name} conjures an iron barrier — the next attack nullified!`;
      }
      break;
    }
    case "poison_dart": {
      ...
      actor.lastCardDmg = Math.max(0, d);
      if (d > 0) { target.poisoned = 8; addLog(room, `☠ ${actor.name} poisoned ${target.name}! ${d} + 8 poison next turn.`); narration = `${actor.name} fires a toxic dart! ${target.name} takes ${d} damage and is POISONED!`; }
      else { addLog(room, `☠ Poison Dart — blocked! No poison.`); narration = `${actor.name}'s dart was blocked!`; }
      break;
    }
    case "taunt": {
      ...
      actor.lastCardDmg = Math.max(0, d);
      const stunned = applyStun(room, actor, target, 1);
      addLog(room, `😤 TAUNT — ${actor.lastCardDmg} DMG${stunned ? " + STUNNED!" : " (stun on cooldown)"}`);
      narration = stunned ? `${actor.name} gets in ${target.name}'s face — ${actor.lastCardDmg} damage + STUNNED!` : `${actor.name} taunts for ${actor.lastCardDmg} damage. Stun on cooldown.`;
      break;
    }
    case "mana_burn": {
      if (target.hand.length > 0) {
        ...
        actor.lastCardDmg = Math.max(0, d);
        addLog(room, `🔥 ${actor.name} burned ${CARD_NAMES[burned]}! +${actor.lastCardDmg} DMG`);
        narration = `${actor.name} ignites ${target.name}'s ${CARD_NAMES[burned]}! +${actor.lastCardDmg} damage.`;
      } else { addLog(room, `🔥 Mana Burn — nothing to burn!`); narration = `Nothing to burn!`; }
      break;
    }
    case "ice_spike": {
      ...
      if (d > 0) {
        const stunned = applyStun(room, actor, target, 1);
        addLog(room, `🧊 ICE SPIKE — ${d} DMG${stunned ? " + stunned!" : " (stun on cooldown!)"}`);
        narration = stunned ? `ICE SPIKE! ${target.name} takes ${d} and is FROZEN!` : `Ice spike for ${d} damage! Stun on cooldown.`;
      } else { addLog(room, `🧊 Ice Spike — blocked!`); narration = `Ice spike shatters harmlessly!`; }
      break;
    }
    case "regen": {
      actor.regenStacks = Math.min(actor.regenStacks + 3, 6);
      addLog(room, `💚 REGEN activated — ${actor.regenStacks * 9} HP/turn!`);
      narration = `${actor.name} activates REGEN — recovering ${actor.regenStacks * 9} HP per turn!`;
      break;
    }
    case "war_cry": {
      actor.warCryBonus = 15;
      addLog(room, `📯 WAR CRY — next attack +15 DMG!`);
      narration = `${actor.name} lets out a WAR CRY! Next strike hits 15 harder!`;
      break;
    }
    // ── RARE ──────────────────────────────────────────────────
    case "gamble": {
      if (Math.random() < 0.5) {
        ...
        narration = `${actor.name} rolls the dice... WON! ${actor.lastCardDmg} damage!`;
      } else {
        actor.hp = Math.max(0, actor.hp - 15);
        addLog(room, `🎲 GAMBLED AND LOST — 15 self-damage!`);
        narration = `${actor.name} rolls the dice... LOST. 15 self-damage!`;
    ...
    case "steal": {
      if (!target.lastCard || target.lastCard === "steal") {
        ...
        narration = `Nothing to mirror!`;
      } else {
        const copyId = target.lastCard;
        // Guard against mirroring time_warp (infinite turns)
        if (copyId === "time_warp") {
          addLog(room, `🪞 ${actor.name} tries to mirror Time Warp — the timeline resists!`);
          narration = `${actor.name} tries to mirror Time Warp... the cosmos won't allow it!`;
        } else {
          addLog(room, `🪞 ${actor.name} mirrors ${CARD_NAMES[copyId]}!`);
          narration = `${actor.name} mirrors ${CARD_NAMES[copyId]}!`;
    ...
    case "divine_shield": {
      actor.shield = true;
      const dh = applyHeal(room, actor, 12);
    ...
    case "double_strike": {
      let total = 0;
      for (let i = 0; i < 2; i++) {
        const d = dealDamage(room, actor, target, 14, `strike ${i+1}`);
        if (d > 0) total += d; else break;
      }
      actor.lastCardDmg = total;
      if (total > 0) { addLog(room, `⚡ DOUBLE STRIKE — ${total} total!`); narration = `${actor.name} strikes TWICE for ${total} total!`; }
      else { addLog(room, `⚡ Double Strike — blocked!`); narration = `Both strikes absorbed!`; }
      break;
    }
    case "vampire_bite": {
      ...
      actor.lastCardDmg = Math.max(0, d);
      if (d > 0) {
        const h = applyHeal(room, actor, Math.floor(d / 2));
        addLog(room, `🧛 VAMPIRE BITE — ${d} drained, +${h} HP!`);
        narration = `${actor.name} drains ${d} HP and restores ${h} to themselves!`;
      } else { addLog(room, `🧛 Vampire Bite — blocked!`); narration = `Blocked — no blood!`; }
      break;
    }
    case "earthquake": {
      const d = dealDamage(room, actor, target, 24, "earthquake");
      actor.lastCardDmg = Math.max(0, d);
      actor.hp = Math.max(0, actor.hp - 8); // self-damage always applies
      addLog(room, `🌍 EARTHQUAKE — ${actor.lastCardDmg} to foe, 8 self-damage!`);
      narration = `EARTHQUAKE! ${target.name} takes ${actor.lastCardDmg}, tremors deal 8 to ${actor.name}!`;
      if (actor.hp <= 0) { endGame(room, targetToken); broadcast(room); return; }
      break;
    }
    case "berserker": {
      ...
      actor.lastCardDmg = Math.max(0, d);
      if (d > 0) { addLog(room, `😡 BERSERKER — ${d} rage damage!`); narration = `${actor.name} BERSERKS! ${d} furious damage!`; }
      else { addLog(room, `😡 Berserker — blocked!`); narration = `Rage absorbed!`; }
      break;
    }
    case "ghost_step": {
      actor.shield = true;
      ...
      narration = `${actor.name} phases through reality! Shield + ${actor.lastCardDmg} counter damage!`;
      break;
    }
    case "reflect": {
      actor.reflectActive = true;
      addLog(room, `🔮 REFLECT set — next hit bounces back!`);
      narration = `${actor.name} sets a reflect mirror!`;
      break;
    }
    case "lucky_shot": {
      const dmg = Math.floor(Math.random() * 26) + 10; // 10-35
      const d = dealDamage(room, actor, target, dmg, "lucky shot");
      actor.lastCardDmg = Math.max(0, d);
      if (d > 0) {
        addLog(room, `🍀 LUCKY SHOT — ${d} damage!${d >= 30 ? " INSANE!" : d <= 12 ? " Rough..." : ""}`);
        narration = d >= 30 ? `LUCKY SHOT hits for ${d} INSANE damage!` : d <= 12 ? `Lucky shot... barely ${d}.` : `Lucky shot! ${d} damage.`;
      } else { addLog(room, `🍀 Lucky Shot — blocked!`); narration = `Even luck can't pierce that!`; }
      break;
    }
    case "counter_stance": {
      actor.counterStanceActive = true;
      addLog(room, `🔄 COUNTER STANCE — next hit reflected + 12 bonus!`);
      narration = `${actor.name} drops into counter stance!`;
      break;
    }
    case "mana_surge": {
      actor.manaSurgeActive = true;
      addLog(room, `✨ MANA SURGE — next card deals double + 10!`);
      narration = `${actor.name} charges mana — next strike: DOUBLE + 10 bonus!`;
      break;
    }
    case "pickpocket": {
      if (target.hand.length > 0) {
        ...
        addLog(room, `🤏 ${actor.name} stole ${CARD_NAMES[stolen]}!`);
        narration = `${actor.name} pickpockets ${CARD_NAMES[stolen]} from ${target.name}!`;
        skipReplenish = true; // already gained a card
      } else { addLog(room, `🤏 Pickpocket — nothing to steal!`); narration = `Nothing to steal!`; }
      break;
    }
    case "explosion": {
      ...
      if (actor.hand.length > 0) {
        const ridx = Math.floor(Math.random() * actor.hand.length);
        const lost = actor.hand.splice(ridx, 1)[0] as CardId;
        addLog(room, `💣 EXPLOSION — ${actor.lastCardDmg} damage! Lost ${CARD_NAMES[lost]}.`);
        narration = `EXPLOSION for ${actor.lastCardDmg}! Blast destroys ${CARD_NAMES[lost]} from own hand!`;
      } else { addLog(room, `💣 EXPLOSION — ${actor.lastCardDmg} damage!`); }
      break;
    }
    case "shockwave": {
      const isStunned = target.skipTurns > 0 || target.stunFlagForChain;
      const totalDmg = isStunned ? 42 : 22;
      const d = dealDamage(room, actor, target, totalDmg, "shockwave");
      actor.lastCardDmg = Math.max(0, d);
      if (isStunned) { addLog(room, `🌊 SHOCKWAVE — ${actor.lastCardDmg} CRUSHING damage (foe stunned)!`); narration = `SHOCKWAVE through the stunned ${target.name}! ${actor.lastCardDmg} CRUSHING damage!`; }
      else { addLog(room, `🌊 SHOCKWAVE — ${actor.lastCardDmg} damage (no stun bonus)`); narration = `Shockwave for ${actor.lastCardDmg}. Stun them first for max damage!`; }
      break;
    }
    case "chain_stun": {
      // Does NOT extend stun — just bonus damage if stunned
      const isStunned = target.skipTurns > 0 || target.stunFlagForChain;
      if (isStunned) {
        const d = dealDamage(room, actor, target, 22, "chain stun");
        actor.lastCardDmg = Math.max(0, d);
        addLog(room, `⛓ CHAIN STUN — foe stunned! ${actor.lastCardDmg} bonus DMG!`);
        narration = `${actor.name} chains the attack on the stunned foe! ${actor.lastCardDmg} bonus damage!`;
      } else {
        const d = dealDamage(room, actor, target, 10, "chain stun");
        actor.lastCardDmg = Math.max(0, d);
        addLog(room, `⛓ Chain Stun — ${actor.lastCardDmg} DMG (no stun active)`);
        narration = `Chain Stun for ${actor.lastCardDmg}. Combo with stun for full power!`;
      }
      break;
    }
    case "sacred_spring": {
      const h = applyHeal(room, actor, 25);
      actor.regenStacks = Math.min(actor.regenStacks + 2, 6);
      addLog(room, `🌿 SACRED SPRING — healed ${h} HP + 2 regen stacks!`);
      narration = `${actor.name} bathes in the Sacred Spring! +${h} HP + 2 regen stacks!`;
      break;
    }
    // ── CURSED ────────────────────────────────────────────────
    case "cursed_blade": {
      const d = dealDamage(room, actor, target, 30, "cursed blade");
      actor.lastCardDmg = Math.max(0, d);
      addLog(room, `🩸 CURSED BLADE — ${actor.lastCardDmg} damage!`);
      narration = `${actor.name} unleashes the Cursed Blade — ${actor.lastCardDmg} dark damage!`;
      break;
    }
    case "blood_pact": {
      actor.hp = Math.max(0, actor.hp - 8);
      if (actor.hp <= 0) { endGame(room, targetToken); broadcast(room); return; }
      const d = dealDamage(room, actor, target, 28, "blood pact");
      actor.lastCardDmg = Math.max(0, d);
      addLog(room, `🩸 BLOOD PACT — sacrifice 8, deal ${actor.lastCardDmg}!`);
      narration = `${actor.name} seals a Blood Pact! Sacrifices 8 HP to unleash ${actor.lastCardDmg} dark damage!`;
      break;
    }
    case "soul_drain": {
      const stacks = target.regenStacks;
      const baseDmg = 10;
      const stackDmg = stacks * 8;
      const totalDmg = baseDmg + stackDmg;
      if (stacks > 0) target.regenStacks = 0;
      const d = dealDamage(room, actor, target, totalDmg, "soul drain");
      actor.lastCardDmg = Math.max(0, d);
      const h = applyHeal(room, actor, Math.floor(actor.lastCardDmg * 0.5));
      addLog(room, `🌙 SOUL DRAIN — ${stacks > 0 ? "drained " + stacks + " regen! " : ""}${actor.lastCardDmg} DMG + ${h} HP stolen!`);
      narration = stacks > 0
        ? `${actor.name} rips ${stacks} regen from ${target.name}'s soul! ${actor.lastCardDmg} damage, ${h} HP stolen!`
        : `${actor.name} drains ${target.name} for ${actor.lastCardDmg} damage and steals ${h} HP!`;
      break;
    }
    // ── CORRUPTED ─────────────────────────────────────────────
    case "cursed_heal": {
      const h1 = applyHeal(room, actor, 35);
      const h2 = applyHeal(room, target, 15);
      addLog(room, `💜 CURSED HEAL — +${h1} HP... but ${target.name} heals ${h2} too!`);
      narration = `${actor.name} heals ${h1} HP! The corruption spreads — ${target.name} also heals ${h2}!`;
      break;
    }
    case "betrayal": {
      ...
      addLog(room, `🗡 BETRAYAL — ${actor.lastCardDmg} to foe, 10 self, gave ${CARD_NAMES[gift]}!`);
      narration = `Betrayal! ${actor.lastCardDmg} damage, 10 self-damage, and gifts ${CARD_NAMES[gift]} to ${target.name}!`;
      if (actor.hp <= 0) { endGame(room, targetToken); broadcast(room); return; }
      break;
    }
    case "plague": {
      // Self takes 5/turn, foe takes 8/turn for 3 turns
      actor.plagueStacks = Math.min(actor.plagueStacks + 3, 6);
      target.plagueStacks = Math.min(target.plagueStacks + 3, 6);
      addLog(room, `🦠 PLAGUE — both infected! You: 5 dmg/turn, foe: 8 dmg/turn for 3 turns!`);
      narration = `${actor.name} releases PLAGUE! You take 5/turn, ${target.name} takes 8/turn for 3 turns!`;
      break;
    }
    case "sacrifice": {
      ...
      actor.hand.push(...drawn);
      skipReplenish = true;
      addLog(room, `💀 SACRIFICE — lose 25 HP, draw 4 cards!`);
      narration = `${actor.name} sacrifices 25 HP to draw 4 new cards!`;
      break;
    }
    case "hand_swap": {
      ...
      skipReplenish = true;
      addLog(room, `🔀 HAND SWAP — hands swapped!`);
      narration = `${actor.name} swaps hands with ${target.name}!`;
      break;
    }
    case "cursed_mirror": {
      ...
      if (target.counterStanceActive)  { actor.counterStanceActive = true; effects.push("Counter"); }
      if (target.manaSurgeActive)      { actor.manaSurgeActive = true; effects.push("Surge"); }
      if (target.warCryBonus > 0)      { actor.warCryBonus = Math.max(actor.warCryBonus, target.warCryBonus); effects.push(`War Cry`); }
      if (target.regenStacks > 0)      { actor.regenStacks = Math.max(actor.regenStacks, target.regenStacks); effects.push(`Regen`); }
      if (target.darkMatterStacks > 0) { actor.darkMatterStacks = Math.max(actor.darkMatterStacks, target.darkMatterStacks); effects.push(`Dark Matter`); }
      ...
      if (effects.length === 0) { addLog(room, `🪟 CURSED MIRROR — nothing to copy!`); narration = `Nothing to copy!`; }
      else { addLog(room, `🪟 CURSED MIRROR — copied: ${effects.join(", ")}!`); narration = `${actor.name} copies: ${effects.join(", ")}!`; }
      break;
    }
    case "curse_tax": {
      if (target.hand.length === 0) { addLog(room, `💸 CURSE TAX — nothing!`); narration = `Nothing to tax!`; }
      else {
        const ridx = Math.floor(Math.random() * target.hand.length);
        ...
        narration = `${actor.name} taxes ${CARD_NAMES[destroyed]} and siphons ${h} HP!`;
      }
      break;
    }
    case "blood_transfusion": {
      const steal = Math.min(22, target.hp - 1); // don't kill
      if (steal <= 0) { addLog(room, `🩺 BLOOD TRANSFUSION — foe too low!`); narration = `Can't drain a dying foe!`; break; }
      target.hp = Math.max(1, target.hp - steal);
      const h = applyHeal(room, actor, steal);
      addLog(room, `🩺 BLOOD TRANSFUSION — stole ${steal} HP! Gained ${h} HP.`);
      narration = `${actor.name} rips ${steal} HP from ${target.name} and absorbs ${h} HP!`;
      break;
    }
    // ── LEGENDARY ─────────────────────────────────────────────
    case "freeze": {
      ...
      addLog(room, `❄ FREEZE — ${actor.lastCardDmg} damage + healing BLOCKED 3 turns!`);
      narration = `${actor.name} encases ${target.name} in cosmic ice — ${actor.lastCardDmg} damage + healing frozen 3 turns!`;
      break;
    }
    case "divine_retribution": {
      ...
      if (d1 > 0) {
        ...
        if (d2 > 0) {
          total += d2;
          const s = applyStun(room, actor, target, 1);
          addLog(room, `⚖ Second smite: ${d2}!${s ? " + STUN!" : " (stun on CD)"}`);
          narration = `DIVINE RETRIBUTION! Two holy smites for ${total} total!${s ? " Second blow STUNS!" : ""}`;
        } else { addLog(room, `⚖ Second smite blocked!`); narration = `Divine Retribution! First smite: ${d1}, second blocked!`; }
      } else { addLog(room, `⚖ DIVINE RETRIBUTION — blocked!`); narration = `Divine Retribution shut down!`; }
      actor.lastCardDmg = total;
      break;
    }
    case "titans_wrath": {
      const diff = Math.abs(actor.hp - target.hp);
      const dmg = Math.min(Math.max(15, diff), 50);
      const d = dealDamage(room, actor, target, dmg, "titan's wrath");
      actor.lastCardDmg = Math.max(0, d);
      addLog(room, `🔱 TITAN'S WRATH — ${actor.lastCardDmg} damage (HP diff: ${diff})!`);
      narration = `TITAN'S WRATH! ${actor.lastCardDmg} damage — greater HP gap = harder strike!`;
      break;
    }
    case "last_stand": {
      // Scaling: (100 - yourHP) * 0.8, capped at 60
      const missing = 100 - actor.hp;
      const dmg = Math.min(Math.floor(missing * 0.8), 60);
      const d = dealDamage(room, actor, target, dmg, "last stand");
      actor.lastCardDmg = Math.max(0, d);
      if (actor.hp <= 40) actor.shield = true;
      addLog(room, `🛡 LAST STAND — ${actor.lastCardDmg} damage${actor.hp <= 40 ? " + SHIELD!" : ""}! (missing ${missing} HP × 0.8)`);
      narration = actor.hp <= 30
        ? `${actor.name} is on the brink! LAST STAND — ${actor.lastCardDmg} MASSIVE damage + shield!`
        : actor.hp <= 40
        ? `${actor.name} fights back! Last Stand — ${actor.lastCardDmg} damage + shield!`
        : `${actor.name} plays Last Stand — ${actor.lastCardDmg} damage. Save it when desperate for max power!`;
      break;
    }
    case "lightning_storm": {
      ...
      for (let i = 0; i < 3; i++) {
        if (Math.random() < 0.70) {
          const d = dealDamage(room, actor, target, 14, "bolt");
          if (d > 0) { stormTotal += d; hits.push(`${d} to ${target.name}`); } else hits.push("blocked!");
      ...
      narration = `LIGHTNING STORM! Three chaotic bolts — ${hits.join(", ")}!`;
      if (actor.hp <= 0) { endGame(room, targetToken); broadcast(room); return; }
      break;
    }
    case "meteor": {
      // 40 damage, bypasses shields, 18 recoil
      const d = dealDamage(room, actor, target, 40, "meteor", true);
      actor.lastCardDmg = Math.max(0, d);
      actor.hp = Math.max(0, actor.hp - 18);
      addLog(room, `☄ METEOR — ${actor.lastCardDmg} damage (bypasses shields)! 18 recoil.`);
      narration = `${actor.name} calls down a METEOR! ${actor.lastCardDmg} catastrophic damage — shields useless! 18 recoil.`;
      if (actor.hp <= 0) { endGame(room, targetToken); broadcast(room); return; }
      break;
    }
    case "overflow": {
      // Draw 5 cards, then next turn player must discard 2 before acting
      const drawn: CardId[] = [];
      for (let i = 0; i < 5; i++) drawn.push(drawOne([...actor.hand as CardId[], ...drawn]));
      actor.hand.push(...drawn);
      actor.overflowPending = true;
      actor.overflowDiscardCount = 2;
      skipReplenish = true;
      addLog(room, `🃏 OVERFLOW — drew 5 cards! Must discard 2 at start of next turn.`);
      narration = `${actor.name} overflows with cards! Drew 5 — must discard 2 at the start of next turn.`;
      break;
    }
    case "echo_strike": {
      if (!actor.lastCard || prevCardDmg <= 0) {
        addLog(room, `🔁 ECHO STRIKE — no previous damage to echo!`);
        narration = `Nothing to echo!`;
      } else {
        ...
        narration = `${actor.name} ECHOES ${CARD_NAMES[actor.lastCard!]} for ${actor.lastCardDmg} damage!`;
      }
      break;
    }
    case "celestial_mend": {
      ...
      const h = applyHeal(room, actor, 55);
      addLog(room, `✨ CELESTIAL MEND — +${h} HP!${cleansed.length ? " Cleansed: " + cleansed.join(", ") + "!" : ""}`);
      narration = cleansed.length
        ? `${actor.name} calls celestial power! +${h} HP and purged ${cleansed.join(", ")}!`
        : `${actor.name} bathed in celestial light — +${h} HP!`;
      break;
    }
    // ── MYTHIC ────────────────────────────────────────────────
    case "soul_swap": {
      const myHp = actor.hp; const thHp = target.hp;
      actor.hp = Math.min(actor.maxHp, thHp);
      target.hp = Math.min(target.maxHp, myHp);
      addLog(room, `👁 SOUL SWAP — HP swapped! ${actor.name}: ${actor.hp}, ${target.name}: ${target.hp}`);
      narration = `SOUL SWAP! HP totals exchanged!`;
      break;
    }
    case "armageddon": {
      actor.shield = false; target.shield = false;
      actor.hp = Math.max(0, actor.hp - 32);
      target.hp = Math.max(0, target.hp - 32);
      addLog(room, `💥 ARMAGEDDON — both take 32 damage! No shields!`);
      narration = `ARMAGEDDON! BOTH players take 32 damage — shields useless!`;
      if (checkDeath(room, actorToken, targetToken)) { broadcast(room); return; }
      break;
    }
    case "void_rift": {
      ...
      skipReplenish = true;
      addLog(room, `🌀 VOID RIFT — all hands destroyed! Both redraw 3!`);
      narration = `VOID RIFT! Both hands consumed — 3 fresh cards each!`;
      break;
    }
    case "judgement": {
      if (target.hp <= 25) {
        ...
        narration = `JUDGEMENT! ${target.name} had only ${target.hp} HP — INSTANTLY DEFEATED!`;
        broadcast(room); return;
      } else {
        const d = dealDamage(room, actor, target, 5, "judgement");
        actor.lastCardDmg = Math.max(0, d);
        addLog(room, `⚖ Judgement... only 5 damage (${target.name} has ${target.hp} HP > 25)`);
        narration = `Judgement... ${target.name} has ${target.hp} HP — too healthy! Only 5 damage.`;
      }
      break;
    }
    case "time_warp": {
      // 2 extra turns, debt 1 skip
      actor.extraTurn = true;
      actor.extraTurnDebt = 1;
      actor.warpTurnsLeft = 2;
      addLog(room, `⏳ TIME WARP — ${actor.name} gets 2 extra turns! Debt: skip 1 later.`);
      narration = `${actor.name} warps time! 2 extra turns NOW — skip 1 turn later as debt!`;
      break;
    }
    case "phoenix_ember": {
      if (actor.hp <= 20) {
        const old = actor.hp;
        actor.hp = Math.min(actor.maxHp, 65);
        actor.shield = true;
        actor.poisoned = 0; actor.plagueStacks = 0; actor.healBlocked = 0;
        actor.skipTurns = 0; actor.neutronStarTicks = 0;
        addLog(room, `🔥 PHOENIX EMBER — risen from ${old} HP to ${actor.hp}! Shield + all debuffs purged!`);
        narration = `${actor.name} was at ${old} HP... the PHOENIX RISES! Restored to ${actor.hp} HP, shield raised, all debuffs purged!`;
      } else {
        const h = applyHeal(room, actor, 22);
        addLog(room, `🔥 Phoenix Ember — healed ${h} HP.`);
        narration = `Phoenix Ember... HP too high for resurrection. +${h} HP.`;
      }
      break;
    }
    // ── VOID ──────────────────────────────────────────────────
    case "singularity": {
      ...
      narration = `SINGULARITY collapses space-time — HP equalized at ${avg}!`;
      break;
    }
    case "dark_matter": {
      actor.darkMatterStacks = 3;
      addLog(room, `🫧 DARK MATTER — next 3 attacks deal 0!`);
      narration = `${actor.name} wraps in DARK MATTER! Next 3 attacks nullified!`;
      break;
    }
    case "event_horizon": {
      actor.eventHorizonActive = true;
      addLog(room, `🌑 EVENT HORIZON — next attack collapses back at attacker!`);
      narration = `${actor.name} bends space! Next attack aimed at them hits the attacker instead!`;
      break;
    }
    case "neutron_star": {
      ...
      addLog(room, `⭐ NEUTRON STAR — ${actor.lastCardDmg} now + 10/turn for 3 turns!`);
      narration = `NEUTRON STAR! ${actor.lastCardDmg} damage + 10 radiation each of ${target.name}'s next 3 turns!`;
      break;
    }
    case "void_pulse": {
      let ec = 0;
      if (target.poisoned > 0) ec++;
      if (target.plagueStacks > 0) ec++;
      if (target.skipTurns > 0) ec++;
      if (target.neutronStarTicks > 0) ec++;
      if (target.eventHorizonActive) ec++;
      if (target.darkMatterStacks > 0) ec++;
      if (target.taunted) ec++;
      if (target.healBlocked > 0) ec++;
      const vpD = Math.min(Math.max(10, ec * 8), 60); // capped at 60
      const d = dealDamage(room, actor, target, vpD, "void pulse");
      actor.lastCardDmg = Math.max(0, d);
      addLog(room, `🔵 VOID PULSE — ${ec} effect(s) × 8 = ${actor.lastCardDmg} damage!`);
      narration = ec >= 3
        ? `VOID PULSE! ${target.name} had ${ec} debuffs — ${actor.lastCardDmg} CRUSHING damage!`
        : `Void Pulse for ${actor.lastCardDmg}. Stack more debuffs for max pain!`;
      break;
    }
  }
  ...
  replenishCard(actor, skipReplenish);

  // Extra turn handling (Time Warp)
  if (actor.extraTurn) {
    actor.extraTurn = false;
    if (actor.warpTurnsLeft > 0) {
      actor.warpTurnsLeft--;
      if (actor.warpTurnsLeft > 0) actor.extraTurn = true;
      addLog(room, `⏳ ${actor.name} takes a TIME WARP turn! (${actor.warpTurnsLeft} left)`);
...
io.on("connection", socket => {
  socket.on("joinRoom", ({ roomId, sessionToken, preferredName }: { roomId: string; sessionToken: string; preferredName: string }) => {
    ...
    const player: Player = {
      ...
      poisoned: 0, regenStacks: 0, plagueStacks: 0, selfPlagueStacks: 0,
      ...
      healBlocked: 0,
      overflowPending: false, overflowDiscardCount: 0,
  ...
  // ── Overflow discard ────────────────────────────────────────
  socket.on("overflowDiscard", (cardIds: CardId[]) => {
    for (const room of rooms.values()) {
      const token = Object.keys(room.players).find(t => room.players[t].socketId === socket.id);
      if (!token) continue;
      const player = room.players[token];
      if (!player.overflowPending) return;
      if (cardIds.length !== 2) return;

      // Remove the 2 chosen cards
      let discarded = 0;
      for (const cid of cardIds) {
        const idx = player.hand.indexOf(cid);
        if (idx !== -1) { player.hand.splice(idx, 1); discarded++; }
      }

      if (discarded === 2) {
        player.overflowPending = false;
        player.overflowDiscardCount = 0;
        addLog(room, `🃏 ${player.name} discarded 2 overflow cards.`);
        room.lastNarration = `${player.name} chose which cards to keep from the overflow!`;
        broadcast(room);
      }
      return;
    }
  ...
  socket.on("playCard", (cardId: CardId) => {
    for (const room of rooms.values()) {
      ...
      const player = room.players[token];
      // Block playing cards while overflow discard is pending
      if (player.overflowPending) {
        socket.emit("overflowRequired", player.hand);
        return;
      }
  ...
  socket.on("drawCard", () => {
    for (const room of rooms.values()) {
      ...
      if (room.gameOver || room.turn !== token) return;
      const player = room.players[token];
      if (player.overflowPending) { socket.emit("overflowRequired", player.hand); return; }
      const oppToken = getOpponentToken(room, token);
      if (player.hand.length >= 8) {
        addLog(room, `🃏 ${player.name}'s hand is full — turn passed!`);
        room.lastNarration = `${player.name} tries to draw but hand is full! Turn wasted.`;
      } else {
        const drawn = drawOne(player.hand as CardId[]);
        player.hand.push(drawn);
        addLog(room, `🃏 ${player.name} draws ${CARD_NAMES[drawn]} and passes!`);
        room.lastNarration = `${player.name} draws ${CARD_NAMES[drawn]} — turn passes!`;
      }
    ...
    for (const room of rooms.values()) {
      ...
      if (tokens.length === 2 && tokens.every(t => room.players[t].rematchReady)) {
        tokens.forEach(t => {
          ...
          p.poisoned = 0; p.regenStacks = 0; p.plagueStacks = 0; p.selfPlagueStacks = 0;
          ...
          p.healBlocked = 0;
          p.overflowPending = false; p.overflowDiscardCount = 0;
